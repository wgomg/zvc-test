class ZBootstrapTable extends ZController {
    onThis_init() {
        ...
        this.isSelectable = !!this.selectedRowClass;
        this.hiddenColumns = {}; // index:true
        this.paginator = null;
        this.pagination = "none";
        this.nPages = 5;
        this.nRowsPerPage = 10;
        const paginationId = this.getAttribute("data-z-pagination");
        if (paginationId) {
        ...
        this.rowsInfo = null;
        const rowsInfoId = this.getAttribute("data-z-rows-info");
        if (rowsInfoId) {
            let searchNode = this.view.parentNode;
        ...
        this.findAll("th").forEach(th => {
            this.columns.push({
                ...
                detailsToggler:!!th.getAttribute("data-z-details-toggler"),
                cellClass:th.getAttribute("data-z-cell-class"),
                clickable:th.getAttribute("data-z-clickable") == "true"
            });
        });
    ...
    async refresh(rowSelector) {
        ...
        if (this.pagination == "none" || this.pagination == "local") {
            ...
            this.repaint();
            if (rowSelector) {
                const idx = this.rows.findIndex(r => rowSelector(r));
    ...
    repaint() {
        const {startRow, endRow} = this.getVisibleRowsRange();
        let html = "";
        ...
        if (this.pagination != "none") {
            const html = this.getPaginatorHTML();
    ...
    getPaginatorHTML() {
        const canPrevious = this.startPage > 1;
        const canNext = (this.startPage + this.nPages - 1) < this.totalPages;
        let html = `
        ...
        while ((this.startPage + n) <= this.totalPages && n < this.nPages) {
            const active = this.startPage + n == this.selectedPage;
            html += `
    ...
    registerEventListeners() {
        ...
        if (this.pagination != "none") {
            ...
            this.paginator.querySelectorAll(".pag-page").forEach(e => {
                e.onclick = evt => {
                    const page = parseInt(e.getAttribute("data-page"));
                    this.pagPage(page);
                    evt.preventDefault();
                }
            })
    ...
    async pagPrevious() {
        await this.closeAllDetails();
        const triggerChange = this.isSelectable && this.selectedRowIndex >= 0;
        this.selectedRowIndex = -1;
        if (this.pagination == "local") {
    ...
    selectRowIndex(idx) {
        const oldSelectedRowIndex = this.selectedRowIndex;
        if (this.selectedRowIndex >= 0) {
            const oldSelected = this.find("tr[data-row='" + this.selectedRowIndex + "']");
            if (oldSelected) oldSelected.classList.remove(this.selectedRowClass);
        }
        this.selectedRowIndex = idx;
        if (idx >= 0) {
            const newSelected = this.find("tr[data-row='" + this.selectedRowIndex + "']");
    ...
    closeAllDetails() {
        return new Promise(resolve => {
            const promises = [];
            const keys = Object.keys(this.detailsPanels);
    ...
    closeDetails(idx) {
        ...
        return new Promise(async (resolve, reject) => {
            const controller = this.detailsPanels[idx];
            await controller.processEvent(controller, "deactivated", []);
            const tr = this.find("tr[data-row='" + idx + "']");
            const detailsRow = tr.nextSibling;
            const detailsDiv = detailsRow.firstChild.firstChild;
            ...
            setTimeout(_ => {
                ...
                const togglerIcon = tr.querySelector(".details-toggler");
    ...
    openDetails(idx) {
        return new Promise(async (resolve, reject) => {
            const spec = await this.triggerEvent("getDetailsConfig", this.rows[idx], idx);
            if (!spec) {
            ...
            const td = newRow.firstChild;
            const detailsDiv = td.firstChild;
            const controller = await ZVC.loadComponent(detailsDiv, this, path);
            const detailsHeight = controller.view.offsetHeight;
            detailsDiv.style["margin-top"] = (-detailsHeight) + "px";
    ...
    openNewDetails(path, panelOptions) {
        return new Promise(async (resolve, reject) => {
            ...
            const firstTr = tbody.firstChild;
            ...
            const contentTd = contentTr.firstChild;
            if (this.detailsCellClass) contentTd.classList.add(this.detailsCellClass);
            const detailsDiv = contentTd.firstChild;
    ...
    closeNewDetails() {
        return new Promise(async (resolve, reject) => {
            ...
            this.newDetailsController = null;        
            const trNewDetailsHeader = this.find(".new-details-header-row");
            const trNewDetailsContent = this.find(".new-details-content-row");
            const detailsDiv = trNewDetailsContent.firstChild.firstChild;
            const detailsHeight = detailsDiv.offsetHeight;
            detailsDiv.style["margin-top"] = (-detailsHeight) + "px";
            setTimeout(_ => {
    ...
    updateRow(idx, row) {
        this.rows[idx] = row;
        const oldTr = this.find("tr[data-row='" + idx + "']");
        const nextTr = oldTr.nextSibling;        
        const newRow = this.getRowElement(row, idx);
        if (this.selectedRowIndex == idx) newRow.classList.add(this.selectedRowClass);
        const tableBody = oldTr.parentNode;
    ...
    async deleteRow(idx) {
        ...
        // Check selection
        const selectionChanged = this.isSelectable && this.selectedRowIndex >= 0;
        this.selectedRowIndex = -1;
        if (this.pagination != "none") {
            this.recalcPagination();
