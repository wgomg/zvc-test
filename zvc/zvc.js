...
class ZVC {
  ...
  static set options(options) {
    ...
    for (const name in options) ZVC._options[name] = options[name];
  }
  ...
  static async fromElement(domElement, options) {
    ...
    if (!path)
      throw "Root element must have data-z-component='path-to-component'";
    if (path.startsWith('./')) {
      let docPath = document.location.pathname;
      if (docPath.toLowerCase().endsWith('.html')) {
        const p = docPath.lastIndexOf('/');
        docPath = docPath.substr(0, p);
      }
      if (docPath.endsWith('/'))
        docPath = docPath.substr(0, docPath.length - 1);
      path = docPath + path.substr(1);
    }
    const newComponent = await ZVC.loadComponent(domElement, null, path);
  ...
  static async loadComponent(domElement, parentController, path) {
    if (path.startsWith('./'))
      path = (parentController ? parentController.path : '') + path.substr(1);
    ...
    const pHtml = fetch(dir + '/' + componentName + '.html', {
      headers: headers,
    });
    const pJs = fetch(dir + '/' + componentName + '.js', { headers: headers });
    try {
      const [resHtml, resJS] = await Promise.all([pHtml, pJs]);
      if (!resHtml.ok) {
        console.error(
          '[' +
            resHtml.status +
      ...
      try {
        ...
        console.error(
          'Error loading controller:' + dir + '/' + componentName + '.js'
        );
        console.error(error);
        console.log(js);
        console.trace(error);
      ...
      const controllerClass = ZVC.lastExportedClass;
      if (!controllerClass)
        throw 'No ZVC.export(ControllerClass) at the end of controller file';
      const zId = ZVC.nextZId();
      if (ZVC.options.htmlPreprocessor)
        html = ZVC.options.htmlPreprocessor(html);
      domElement.innerHTML = ZVC.parseHTML(html, { zId: zId });
      const controller = new controllerClass(domElement, parentController, dir);
      controller.zId = zId;
      return controller;
    } catch (error) {
      console.trace(error);
  ...
  static parseHTML(html, vars) {
    ...
    while (i1 >= 0) {
      const p = html.indexOf('}', i1);
      if (p >= 0) {
        st += html.substring(i0, i1);
        const varName = html.substring(i1 + 2, p);
  ...
  static async openDialogInPlatform() {
    throw 'openDialogInPlatform not implemented for ZDialogs';
  }

  static async closeDialogInPlatform() {
    throw 'closeDialogInPlatform not implemented for ZDialogs';
  }
  ...
  static createTemporaryAnimation(name, frames, autodeleteDelay) {
    ...
    setTimeout(
      (_) => styleElement.parentNode.removeChild(styleElement),
      autodeleteDelay
    );
  }
}
...
class ZController {
  ...
  async processEvent(control, eventName, args) {
    const srcId = control == this ? 'this' : control.id;
    const funcName =
      'on' +
      srcId.substr(0, 1).toUpperCase() +
      srcId.substr(1) +
      '_' +
      eventName;
  ...
  show(display) {
    const newDisplay = display || this.savedDisplay || 'block';
  ...
  isEnabled() {
    return !this.view.disableb;
  }

  // get pos() {let r = this.view.getBoundingClientRect(); return {left:r.left, top:r.top}}
  get pos() {
    return {
      left: parseFloat(this.view.style.left),
      top: parseFloat(this.view.style.top),
    };
  }

  set pos(p) {
    this.view.style.left = p.left + 'px';
    this.view.style.top = p.top + 'px';
  }

  get size() {
    return { width: this.view.clientWidth, height: this.view.clientHeight };
  }

  set size(s) {
    this.view.style.width = s.width + 'px';
    this.view.style.height = s.height + 'px';
  }
  ...
  async showDialog(path, options, okCallback, cancelCallback) {
    ...
    cnt.appendChild(div);
    const controller = await ZVC.loadComponent(div, this, path);
    if (!controller instanceof ZDialog)
      throw 'Controller is not a ZDialog instance';
...
class ZBasicController extends ZController {
  ...
  initDefaults() {
    if (this.getAttribute('data-z-clickable'))
...
class ZSelect extends ZController {
  ...
  get selectedText() {
    const idx = this.view.selectedIndex;
...
ZVC.registerComponent(
  'SELECT',
  (e) =>
    !e.getAttribute('data-z-id-field') || !e.getAttribute('data-z-label-field'),
  ZSelect
);
...
class ZDynamicSelect extends ZSelect {
  ...
  get selectedRow() {
    const idx = parseInt(this.view.value);
    if (isNaN(idx) || idx < 0) return null;
  ...
  set value(v) {
    const idx = this.rows.findIndex((r) => r[this.idField] == v);
    if (idx >= 0) this.view.value = '' + idx;
  }

  setRows(rows, selectedId = null) {
    this.rows = rows;
    this.view.innerHTML = this.rows.reduce(
      (html, row, idx) => {
        const id = row[this.idField];
        const label = window.toLang
          ? window.toLang(row[this.labelField])
          : row[this.labelField];
        html +=
          "<option value='" +
          idx +
...
class ZLoader extends ZController {
  async init() {
    const initialComponentPath = this.view.getAttribute('data-z-load');
    this.content = await ZVC.loadComponent(
      this.view,
      this,
  ...
  async processEvent(control, eventName, args) {
    if (control == this)
      return await super.processEvent(control, eventName, args);
